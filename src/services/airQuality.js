...
// European Air Quality Index bands, ordered from best to worst. Upper bounds
// are inclusive. Source of the band boundaries: the European Environment
// Agency EAQI definition used by the Open-Meteo air-quality API.
...
// Values are clamped rather than rejected: the upstream API occasionally
// returns small negative numbers for very clean air, and treating those as an
// error would hide an otherwise valid reading from the user.
...
// The front-end never sees the raw upstream response — keeping the
// transformation on the server means the browser contract stays stable even
// if the upstream field names change, and it satisfies the project constraint
// that external APIs are consumed by the back-end only.
...
// Errors are tagged with `.status` (404 unknown place, 502 upstream failure)
// so the route handler can map them straight onto an HTTP response.
